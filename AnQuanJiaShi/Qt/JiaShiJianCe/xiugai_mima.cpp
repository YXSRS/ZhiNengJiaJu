...
XiuGai_MiMa::XiuGai_MiMa(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::XiuGai_MiMa)
{
    ui->setupUi(this);
    //设置窗口标题
    setWindowTitle("修改密码");
    ...
    connect(ui->btn_QueRen,&QPushButton::clicked,[=](){
        QString zhang_hao=ui->bjk_ZhangHao_2->text();//获取用户输入的帐号
        QString mi_ma1=ui->bjk_XinMiMa1_2->text();//获取用户输入的密码1
        QString mi_ma2=ui->bjk_XinMiMa2_2->text();//获取用户输入的密码2
