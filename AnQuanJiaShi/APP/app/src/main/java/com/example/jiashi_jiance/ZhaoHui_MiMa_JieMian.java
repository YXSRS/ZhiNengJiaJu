...
public class ZhaoHui_MiMa_JieMian extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_zhao_hui_mi_ma_jie_mian);
                ...
                if ((!zhang_hao.equals(""))&&(!xin_MiMa1.equals(""))&&(!xin_MiMa2.equals(""))) {//都不为空
                    if (xin_MiMa1.equals(xin_MiMa2)) {//两次输入密码一致
                        ...
                        if (flag == 1) {//该账号存在
                            ...
                            //弹出提示信息
                            AlertDialog textTips = new AlertDialog.Builder(ZhaoHui_MiMa_JieMian.this).setTitle("提示").setMessage("密码修改成功！").create();
                            textTips.show();
                        } else {
                            //弹出提示信息
                            AlertDialog textTips = new AlertDialog.Builder(ZhaoHui_MiMa_JieMian.this).setTitle("提示").setMessage("该账号不存在！").create();
                            textTips.show();
                        }
                    } else {
                        //弹出提示信息
                        AlertDialog textTips = new AlertDialog.Builder(ZhaoHui_MiMa_JieMian.this).setTitle("提示").setMessage("两次输入的密码不一致！").create();
                        textTips.show();
                    }
                } else {
                    //弹出提示信息
                    AlertDialog textTips = new AlertDialog.Builder(ZhaoHui_MiMa_JieMian.this).setTitle("提示").setMessage("请填完整信息，信息不能为空！").create();
                    textTips.show();
                }
                break;
            }
