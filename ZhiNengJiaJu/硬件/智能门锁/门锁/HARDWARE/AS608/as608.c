...
* @Features      ：判断中断接收的数组有没有应答包
* @InportFunction：waittime为等待中断接收数据的时间（单位1ms）           
* @ExportFunction：返回值：数据包首地址
* @Author        ：TXD
* @ModifiedDate  ：2020.12.29
* @Remarks       ：直接调用
**********************************************************************/
...
* @Features      ：探测手指，探测到后录入指纹图像存于ImageBuffer。
* @InportFunction：waittime为等待中断接收数据的时间（单位1ms）           
* @ExportFunction：模块返回确认字
* @Author        ：TXD
* @ModifiedDate  ：2020.12.29
* @Remarks       ：录入图像 PS_GetImage
**********************************************************************/
u8 PS_GetImage(void)
{
  u16 temp;
...
{//数据包格式
  ...
  SendHead();//包头
  SendAddr();//芯片地址
  SendFlag(0x01);//命令包标识，将传感器上读入图像存于图像缓冲区
  SendLength(0x06);
  Sendcmd(0x06);//将特征缓冲区中的文件储存到 flash 指纹库中
  MYUSART_SendData(BufferID);
...
* @Features      ：以 CharBuffer1或CharBuffer2中的特征文件高速搜索整个或部分指纹库。
		           若搜索到，则返回页码,该指令对于的确存在于指纹库中 ，且登录时质量
		           很好的指纹，会很快给出搜索结果。
* @InportFunction：BufferID， StartPage(起始页)，PageNum（页数）
* @ExportFunction：模块返回确认字+页码（相配指纹模板）
* @Author        ：TXD
* @ModifiedDate  ：2020.12.29
* @Remarks       ：高速搜索PS_HighSpeedSearch
**********************************************************************/
...
* @Features      ：读有效模板个数
* @InportFunction：No
* @ExportFunction：模块返回确认字+有效模板个数ValidN
* @Author        ：TXD
* @ModifiedDate  ：2020.12.29
* @Remarks       ：读有效模板个数 PS_ValidTempleteNum
**********************************************************************/
u8 ensure_num=0;
...
* @Features      ：与AS608握手 PS_HandShake
* @InportFunction：PS_Addr地址指针
* @ExportFunction：模块返新地址（正确地址）
* @Author        ：TXD
* @ModifiedDate  ：2020.12.29
* @Remarks       ：No
**********************************************************************/
