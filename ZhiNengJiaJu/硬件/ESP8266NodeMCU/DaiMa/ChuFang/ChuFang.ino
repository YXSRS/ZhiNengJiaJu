...
IPAddress local_IP(10, 3, 22, 16); // 设置ESP8266-NodeMCU联网后的IP
...
void setup() {
    ...
    Serial.println("ChuFang_esp8266_server,启动");
...
void loop() {
    ...
    if(ZM_ChuFang_old != ZM_ChuFang){
        if(ZM_ChuFang == 0){
            digitalWrite(D1,LOW);
            digitalWrite(D2,LOW);
        }
        else if(ZM_ChuFang == 1){
            digitalWrite(D1,HIGH);
            digitalWrite(D2,LOW);
        }
        else if(ZM_ChuFang == 2){
            digitalWrite(D1,LOW);
            digitalWrite(D2,HIGH);
        }
        else if(ZM_ChuFang == 3){
            digitalWrite(D1,HIGH);
            digitalWrite(D2,HIGH);
        }
        ZM_ChuFang_old = ZM_ChuFang;
    }
}

void XiangYing_FWQ(){
    ZM_ChuFang = esp8266_server.arg("ZM_ChuFang").toInt();
    Serial.print("ZM_ChuFang:");
    Serial.println(ZM_ChuFang);
}
