...
IPAddress local_IP(10, 3, 22, 13); // 设置ESP8266-NodeMCU联网后的IP
...
int AF_YuDi = -1;
int AF_ChuangHu = -1;
int AF_ChuangHu_old = -1;
int AF_JiaJuDianQi1 = -1;
int AF_JiaJuDianQi1_old = -1;

const uint16_t kIrLed = 16;  // ESP8266 GPIO pin to use. Recommended: 14 (D0).
...
void setup() {
    ...
    Serial.println("ZhuWo_esp8266_server,启动");

    pinMode(D1, OUTPUT);
    digitalWrite(D1,LOW);
    pinMode(D2, OUTPUT);
    digitalWrite(D2,LOW);
    pinMode(D5,OUTPUT);//窗户
    digitalWrite(D5,LOW);
    pinMode(D6, OUTPUT);//家居电器1
    digitalWrite(D6,LOW);
    pinMode(D7, INPUT_PULLUP);//雨滴
...
void loop() {
    ...
    if(ZM_ZhuWo_old != ZM_ZhuWo){
        ...
        ZM_ZhuWo_old = ZM_ZhuWo;
    }
    if(AF_ChuangHu_old!=AF_ChuangHu){
        if(AF_ChuangHu==1){
    ...
    if(AF_JiaJuDianQi1_old!=AF_JiaJuDianQi1){
        if(AF_JiaJuDianQi1==1){
    ...
    if(NT_ZhuWo_old!=NT_ZhuWo){
        NuanTong_do(NT_ZhuWo);
        NT_ZhuWo_old=NT_ZhuWo;
    }
    if(AF_ChuangHu_old!=AF_ChuangHu){
        if(AF_ChuangHu==1){
            DuoJi.write(90);
        ...
        AF_ChuangHu_old=AF_ChuangHu;
    }
    AF_ZhuWo();
}

void XiangYing_FWQ(){
    ZM_ZhuWo=esp8266_server.arg("ZM_ZhuWo").toInt();
    Serial.print("ZM_ZhuWo:");
    Serial.println(ZM_ZhuWo);

    NT_ZhuWo=esp8266_server.arg("NT_ZhuWo").toInt();
    Serial.print("NT_ZhuWo:");
    Serial.println(NT_ZhuWo);

    AF_ChuangHu = esp8266_server.arg("AF_ChuangHu").toInt();
    Serial.print("AF_ChuangHu:");
    Serial.println(AF_ChuangHu);

    AF_JiaJuDianQi1 = esp8266_server.arg("AF_JiaJuDianQi1").toInt();
    Serial.print("AF_JiaJuDianQi1:");
    Serial.println(AF_JiaJuDianQi1);
}
...
void AF_ZhuWo(){
    ...
    if(i>=2200){
        i=0;
        AF_YuDi = digitalRead(D7);
        Serial.print("AF_YuDi:");
        Serial.println(AF_YuDi);
        PostData();
    }
}
...
void PostData(){
    ...
    // Execute the query
    char sql[100];
    sprintf(sql,"update anfang_biao set YuDi=%d where id=1;",!AF_YuDi);
