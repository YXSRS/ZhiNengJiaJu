...
IPAddress local_IP(10, 3, 22, 15); // 设置ESP8266-NodeMCU联网后的IP
...
int ZM_ShuFang=-1;
int ZM_ShuFang_old=-1;
int NT_ShuFang=-1;
int NT_ShuFang_old=-1;
bool HuoYan_pin;
int HuoYan;
int MieHuoQi;
...
void setup(){
    ...
    Serial.println("ShuFang_esp8266_server,启动");

    pinMode(D1, OUTPUT);
    digitalWrite(D1,LOW);
    pinMode(D2, OUTPUT);
    digitalWrite(D2,LOW);
    pinMode(D5, INPUT_PULLUP);//火焰
    pinMode(D6, OUTPUT);//灭火器
    digitalWrite(D6,LOW);
...
void loop() {
    ...
    if(ZM_ShuFang_old!=ZM_ShuFang){
        if(ZM_ShuFang==0){
            digitalWrite(D1,LOW);
            digitalWrite(D2,LOW);
        }
        else if(ZM_ShuFang==1){
            digitalWrite(D1,HIGH);
            digitalWrite(D2,LOW);
        }
        else if(ZM_ShuFang==2){
            digitalWrite(D1,LOW);
            digitalWrite(D2,HIGH);
        }
        else if(ZM_ShuFang==3){
            digitalWrite(D1,HIGH);
            digitalWrite(D2,HIGH);
        }
        ZM_ShuFang_old=ZM_ShuFang;
    }

    if(NT_ShuFang_old!=NT_ShuFang){
        NuanTong_do(NT_ShuFang);
        NT_ShuFang_old=NT_ShuFang;
    }
...
void XiangYing_FWQ(){
    ZM_ShuFang=esp8266_server.arg("ZM_ShuFang").toInt();
    NT_ShuFang=esp8266_server.arg("NT_ShuFang").toInt();
    Serial.print("ZM_ShuFang:");
    Serial.println(ZM_ShuFang);
    Serial.print("NT_ShuFang:");
...
void PostData(){
    ...
    // Execute the query
    char sql[100];
    sprintf(sql,"update anfang_biao set HuoYan=%d, MieHuoQi=%d where id=1;",HuoYan,MieHuoQi);
