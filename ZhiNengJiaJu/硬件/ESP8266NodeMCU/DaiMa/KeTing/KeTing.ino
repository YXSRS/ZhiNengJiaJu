...
IPAddress local_IP(10, 3, 22, 11); // 设置ESP8266-NodeMCU联网后的IP
...
int NT_KeTing=-1;
int NT_KeTing_old=-1;
float ShiNei_WenDu=0.0;
float ShiNei_ShiDu=0.0;
...
void setup(){
    ...
    Serial.println("KeTing_esp8266_server,启动");
...
void loop() {
    ...
    if(ZM_KeTing_old!=ZM_KeTing){
        ...
        ZM_KeTing_old=ZM_KeTing;
    }
    if(NT_KeTing_old!=NT_KeTing){
        NuanTong_do(NT_KeTing);
        NT_KeTing_old=NT_KeTing;
    }
    bme280();
}

void XiangYing_FWQ(){
    ZM_KeTing=esp8266_server.arg("ZM_KeTing").toInt();
    Serial.print("ZM_KeTing:");
    Serial.println(ZM_KeTing);
    
    NT_KeTing=esp8266_server.arg("NT_KeTing").toInt();
    Serial.print("NT_KeTing:");
    Serial.println(NT_KeTing);
}
...
void bme280(){
    ...
    if(i>=2200){
        i=0;
        ShiNei_WenDu = bme.readTemperature(); // 读取温度数据
        Serial.print("ShiNei_WenDu=");
        Serial.print(ShiNei_WenDu);

        ShiNei_ShiDu = bme.readHumidity();       // 读取湿度数据
        Serial.print("ShiNei_ShiDu=");
        Serial.print(ShiNei_ShiDu);
        PostData();
    }
}
...
void PostData(){
    ...
    // Execute the query
    char sql[100];
    sprintf(sql,"update anfang_biao set ShiNei_WenDu=%.2f, ShiNei_ShiDu=%.2f where id=1;",ShiNei_WenDu,ShiNei_ShiDu);
